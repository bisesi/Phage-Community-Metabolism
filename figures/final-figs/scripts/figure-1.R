...
pfba <- read_csv("pfba_fluxes_optimized.csv", show_col_types = FALSE) %>% rename(reaction = `...1`)
fva <- read_csv("fva_fluxes_optimized.csv", show_col_types = FALSE) %>% rename(reaction = `...1`)
...
pie_charts <- reactions %>% full_join(., rbind(nucleic_acid, amino_acids, ions, vitamin, lipids, hemes), by = "Chemical") %>%
  ...
  ungroup() %>% mutate(percent = sub / total) %>% mutate(percent = case_when(reaction == "plasmid" & type == "amino acids" ~ 0.995,
                                                                             ...
                                                                             TRUE ~ percent)) %>%
  mutate(reaction = ifelse(reaction == "host", "bacterial biomass", reaction)) %>%
  mutate(reaction = ifelse(reaction == "plasmid", "plasmid biomass", reaction)) %>% 
  mutate(reaction = ifelse(reaction == "virus", "phage biomass", reaction)) %>%
  mutate(reaction = factor(reaction, levels = c("bacterial biomass", "plasmid biomass", "phage biomass"))) %>%
...
partC <- data.frame(comparison = c("plasmid", "virus with plasmid"),
...
# part D - percent non overlapping per subprocess
partD <- plasmid_by_process %>%
...
# final figure
top <- plot_grid(partA, partB, ncol = 1, rel_heights= c(1,0.7), label_size = 26, labels = c("A", "B"))
middle <- plot_grid(partC, partD, ncol = 2, rel_widths = c(0.3,1), label_size = 26, labels = c("C", "D"))
bottom <- plot_grid(partE, partF, ncol = 2, rel_widths = c(1,0.5), label_size = 26, labels = c("E", "F"))
figure1 <- plot_grid(top, plot_grid(middle, bottom, ncol = 1, rel_heights = c(0.5,1)), ncol = 2, rel_widths = c(0.5,1))

png(here::here("figures", "final-figs", "imgs", "figure-1.png"), res = 300, width = 5100, height = 2900)
figure1
dev.off()
