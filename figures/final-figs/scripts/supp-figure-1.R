...
bounds <- es %>% mutate(optimized = ifelse(optimized == "virus" & lower_bound == 0, "host", optimized)) %>% pull(lower_bound) %>% unique()

plasmid_bound <- bounds[6]

virus_bound <- bounds[17]

partA <- rbind(es,em,esm) %>%
  mutate(optimized = ifelse(optimized == "virus" & lower_bound == 0, "host", optimized)) %>% 
  ...
  geom_line(linewidth = 2) + facet_grid(interaction~optimized) + theme_bw(base_size = 16) +
  theme(axis.text = element_markdown(),
        legend.position = "none", strip.background = element_blank(),
        strip.text = element_markdown())

# part B - percent partner
...
partC <- rbind(es,em,esm) %>%
  ...
  mutate(interaction = factor(interaction, levels = c("*S. enterica*", "*M. extorquens*", "*S. enterica*<br>+<br>*M. extorquens*"))) %>%
  ggplot(aes(x = optimized, y = total_yield, color = optimized)) +
  geom_point(size = 2, stroke = 1.5) + facet_wrap(~interaction) +
  scale_color_manual(values = c("uninf" = "#F5793A", "F128+" = "#A95AA1", "F128+<br>M13+" = "#0F2080")) +
  theme_bw(base_size = 16)+
  ylab("maximum total biomass") +
  theme(axis.text = element_markdown(), axis.title.x = element_blank(),
        legend.position = "none", strip.background = element_blank(), strip.text = element_markdown())

# part D - what are the partners using
...
compounds_SM <- c("EX_mea_e", "EX_succ_e", "EX_ac_e", "EX_glyc_e", "EX_nh4_e") 

compounds_E <- c("EX_ac_e", "EX_ade_e", "EX_dha_e", "EX_etoh_e", "EX_glyc_e", "EX_succ_e", "EX_g3pg_e", "EX_glyc3p_e", 
                 "EX_glyc__R_e", "EX_ala__L_e", "EX_gal_e", "EX_xan_e", "EX_o16a4colipa_e") 

shared <- c("acetate", "succinate", "glycerol")
...
uptake <- rbind(es_flux, esm_flux, em_flux) %>%
  ...
  mutate(name = case_when(name == "EX_ac_e" ~ "acetate",
                          ...
                          name == "EX_glyc_e" ~ "glycerol",
                          name == "EX_mea_e" ~ "methylamine",
                          name == "EX_nh4_e" ~ "nitrate")) %>%
...
partD <- uptake %>%
  ...
  scale_fill_manual(values = c("uninf" = "#F5793A", "F128+" = "#A95AA1", "F128+<br>M13+" = "#0F2080")) +
  theme_bw(base_size = 16)+
...
# part E - was is E spitting out
secretion <- rbind(es_flux, esm_flux, em_flux) %>%
  filter(species == "iJO1366") %>%
  mutate(optimized = ifelse(optimized == "virus" & lower_bound == 0, "host", optimized)) %>% 
  filter(lower_bound %in% c(0, virus_bound, plasmid_bound)) %>%
  filter(optimized != "virus") %>%
  filter(!(lower_bound == 0 & optimized == "plasmid") & !(lower_bound == 0 & optimized == "virus with plasmid")) %>%
  filter(flux != 0) %>%
  group_by(species, optimized, name, interaction) %>%
  ...
  filter(interaction != "*S. enterica*<br>+<br>*M. extorquens*") %>%
  mutate(interaction = factor(interaction, levels = c("*S. enterica*", "*M. extorquens*"))) %>%
  mutate(name = case_when(name == "EX_xan_e" ~ "xanthine",
                          name == "EX_ac_e" ~ "acetate",
                          name == "EX_ade_e" ~ "adenine",
                          name == "EX_etoh_e" ~ "ethanol",
                          name == "EX_g3pg_e" ~ "glycerophosphoglycerol",
                          name == "EX_glyc_e" ~ "glycerol",
                          name == "EX_succ_e" ~ "succinate",
                          name == "EX_ala__L_e" ~ "alanine",
                          name == "EX_gal_e" ~ "galactose",
                          name == "EX_glyc3p_e" ~ "glycerol 3-phosphate",
                          name == "EX_dha_e" ~ "dihydroxyacetone",
                          name == "EX_o16a4colipa_e" ~ "lipid A",
                          name == "EX_glyc__R_e" ~ "glycerate")) %>%
  mutate(name = ifelse(name %in% shared, paste0("***", name, "***"), name))
...
top <- plot_grid(partA, left, ncol = 2, label_size = 26, labels = c("A", ""), rel_widths = c(0.8,1))
bottom <- plot_grid(partE, partD, ncol = 2, labels = c("D", "E"), label_size = 26, rel_widths = c(1,0.7))
suppfigure1 <- plot_grid(top, bottom, ncol = 1, rel_heights = c(1, 0.7))

png(here::here("figures", "final-figs", "imgs", "supp-figure-1.png"), res = 300, width = 4250, height = 3000)
suppfigure1
dev.off()
